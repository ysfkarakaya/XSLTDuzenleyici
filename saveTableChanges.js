...
function isSimilarTable(tableHTML, tableData) {
    ...
    // Tablo boyutları farklı olabilir (yeni satırlar eklenmiş olabilir)
    // Bu nedenle, en az bir satırın benzer olup olmadığını kontrol et
    ...
    // En az 1 benzer satır varsa veya orijinal tablo boşsa true döndür
    return similarRowCount > 0 || extractedData.length === 0;
}
...
function createTableHTML(editableTable, originalTable) {
    ...
    editableRows.forEach((editableRow, rowIndex) => {
        ...
        editableCells.forEach((cell) => {
            ...
            Array.from(cell.parentNode.attributes)
            ...
            // Textarea'dan içeriği al
            const textarea = cell.closest('td').querySelector('textarea');
            let content = '';
            
            if (textarea) {
                content = textarea.value;
                
                // \n karakterlerini <br/> etiketlerine dönüştür
                // &nbsp; karakterlerini &#160; karakterlerine dönüştür
                content = content
                    .replace(/&nbsp;/g, '&#160;')
                    .replace(/\n/g, '<br/>')
